-- ─────────────────────────────────────────────────────────────
-- F89 — site_settings 초기 시드
-- 약관·방침 본문은 기존 JSX(app/(public)/terms·privacy/page.tsx)를 HTML로 변환.
-- {{service_name}} {{company_name}} {{contact_email}} placeholder는 server에서 치환.
-- footer_address는 빈 값 (사용자 결정 대기, Sprint C 편집 비활성)
-- ─────────────────────────────────────────────────────────────

INSERT INTO public.site_settings (key, value, description)
VALUES
  ('service_name',    '가짜생각',                    '서비스명'),
  ('company_name',    '마인드시어터',                '회사명'),
  ('contact_email',   'support@mindreframe.net',     '대표 이메일'),
  ('effective_date',  '2026-05-01',                  '약관·방침 시행일'),
  ('footer_address',  '',                            '회사 주소 (Sprint C 보류 — 사용자 결정 후)'),
  ('terms_html', $TERMS$
...
$TERMS$, '이용약관 본문 (HTML, {{}} placeholder 지원)'),
...
$PRIVACY$, '개인정보처리방침 본문 (HTML, {{}} placeholder 지원)')
