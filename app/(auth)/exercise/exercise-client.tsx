...
import {
  ...
  type AnxStep4,
  type DepStep4,
} from "@/lib/actions/exercise-state";
...
export interface ExerciseClientProps {
  heroSubtitle?: string;
  initialState: ExerciseStateRow | null;
  popups: {
    step1: PopupContent | null;
    step2: PopupContent | null;
    step3: PopupContent | null;
    step4Praise: PopupContent | null;
  };
}
...
export function ExerciseClient({
  ...
  // DB → localStorage → buildXXX 순으로 hydration
  const dbAnxRows: AnxietyPlanItem[] | null = initialState?.anxPlan ?? null;
  const dbDepRows: DepressPlanItem[] | null = initialState?.depPlan ?? null;

  const [mode, setMode] = useState<ExerciseMode | null>(initialState?.mode ?? null);
  const [anxRows, setAnxRows] = useState<AnxietyPlanItem[]>(() => {
    if (dbAnxRows && dbAnxRows.length > 0) {
      const r = buildAnxRows();
      dbAnxRows.slice(0, 10).forEach((row, i) => {
        r[i] = {
  ...
  const [depRows, setDepRows] = useState<DepressPlanItem[]>(() => {
    if (dbDepRows && dbDepRows.length > 0) {
      const r = buildDepRows();
      dbDepRows.slice(0, 10).forEach((row, i) => {
        r[i] = {
  ...
  const [anxSaved, setAnxSaved] = useState(initialState?.anxSaved ?? false);
  const [depSaved, setDepSaved] = useState(initialState?.depSaved ?? false);
  const [anxSelectedIdx, setAnxSelectedIdx] = useState<number | null>(
    initialState?.anxSelectedIdx ?? null,
  );
  const [depSelectedIdx, setDepSelectedIdx] = useState<number | null>(
    initialState?.depSelectedIdx ?? null,
  );
  const [step4Open, setStep4Open] = useState(initialState?.step4Open ?? false);

  // 4단계 폼 — 불안 (DB step4 hydration)
  const [anxDid, setAnxDid] = useState<"did" | "no">(
    initialState?.anxStep4?.did ?? "did",
  );
  const [anxBefore, setAnxBefore] = useState(initialState?.anxStep4?.before ?? "");
  const [anxAfter, setAnxAfter] = useState(initialState?.anxStep4?.after ?? "");
  const [anxLearned, setAnxLearned] = useState(initialState?.anxStep4?.learned ?? "");
  const [anxUnexpected, setAnxUnexpected] = useState(
    initialState?.anxStep4?.unexpected ?? "",
  );

  // 4단계 폼 — 우울
  const [depDid, setDepDid] = useState<"did" | "no">(
    initialState?.depStep4?.did ?? "did",
  );
  const [depAfterMood, setDepAfterMood] = useState(
    initialState?.depStep4?.afterMood ?? "",
  );
  const [depLearned, setDepLearned] = useState(initialState?.depStep4?.learned ?? "");
  const [depUnexpected, setDepUnexpected] = useState(
    initialState?.depStep4?.unexpected ?? "",
  );
  ...
  // 초기 fallback hydration — DB가 비었으면 localStorage에서 복원
  /* eslint-disable react-hooks/set-state-in-effect */
  useEffect(() => {
    if (initialState) return; // DB 있음 → 그대로 사용
  ...
  // localStorage 자동 저장 (debounce 180ms)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    dbSaveTimer.current = setTimeout(() => {
      const anxStep4: AnxStep4 = {
        did: anxDid,
        before: anxBefore,
        after: anxAfter,
        learned: anxLearned,
        unexpected: anxUnexpected,
      };
      const depStep4: DepStep4 = {
        did: depDid,
        afterMood: depAfterMood,
        learned: depLearned,
        unexpected: depUnexpected,
      };
  ...
  function sortAndPersistAnxPlan() {
    const items = anxRows
  ...
  if (!mode) {
    return (
      <PageFade>
        ...
        {popups.step1 ? (
          <GamePopup
            storageKey="popup_exercise_step1_dismissed_at"
            title={popups.step1.title}
            body={popups.step1.body}
            ctaLabel={popups.step1.cta ?? "시작하기"}
            variant="gold-border"
          />
        ) : null}
    ...
    <PageFade>
      ...
      {popups.step2 && !planSaved ? (
        <GamePopup
          ...
          body={popups.step2.body}
          ctaLabel={popups.step2.cta ?? "네, 적어볼게요"}
          variant="navy"
        />
      ) : null}
      ...
      <main className="max-w-[1120px] mx-auto px-4 pt-8 md:pt-10 pb-24">
        ...
        <Card className="shadow-toss-card">
          ...
          <div className="mt-4 flex gap-2 flex-wrap items-center">
            <button
              type="button"
              onClick={isAnxiety ? sortAndPersistAnxPlan : sortAndPersistDepPlan}
