...
export async function adminSetCouponActive(code: string, active: boolean) {
  const g = await ensureAdmin();
  if (!g.ok) return g;
  ...
  await writeAudit({
    adminUserId: g.userId,
    action: active ? "coupon.update" : "coupon.deactivate",
    payload: { code, isActive: active },
  });
...
export type AdminUpdateCouponInput = {
  description?: string | null;
  plan: "light" | "pro" | "premium";
  durationDays: number;
  validFrom?: string | null;
...
export async function adminUpdateCoupon(code: string, input: AdminUpdateCouponInput) {
  const g = await ensureAdmin();
  if (!g.ok) return g;
...
export async function adminDeleteCoupon(code: string) {
  const g = await ensureAdmin();
  if (!g.ok) return g;

  const { error } = await supabaseAdmin.from("coupons").delete().eq("code", code);
  if (error) {
    // 사용 이력(coupon_redemptions) FK 참조 시 삭제 불가
    if ((error as { code?: string }).code === "23503") {
      return {
        ok: false as const,
        error: "사용 이력이 있는 쿠폰은 삭제할 수 없어요. 비활성화하세요",
      };
    }
    if ((error as { code?: string }).code === "42P01") {
      return { ok: false as const, error: "마이그레이션 적용 후 사용 가능해요" };
